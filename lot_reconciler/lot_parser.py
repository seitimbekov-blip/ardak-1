"""Парсинг номера лота вида '56/100 Т', '56/100-1 Т', '29/20-1 Т', 'CZ/755 Т'.

Формат: <базовый_номер>[-<версия>] <вид_закупки>
    базовый_номер  - "56/100" (число/число) или с буквенным префиксом "CZ/755"
                      (встречается в реальных выгрузках для отдельной категории лотов)
...
_LOT_RE = re.compile(
    r"^\s*(?P<base>[A-Za-zА-Яа-я0-9]+\s*/\s*\d+)\s*(?:-\s*(?P<version>\d+))?"
    r"\s*(?P<kind>[ТРУтру])\s*\.?\s*$"
)
...
def parse_lot_number(raw_value) -> LotIdentifier:
    ...
    if not match:
        ...
        loose_match = re.match(
            r"^\s*(?P<base>[A-Za-zА-Яа-я0-9]+\s*/\s*\d+)\s*(?:-\s*(?P<version>\d+))?\s*\.?\s*$",
            raw_str,
        )
        if loose_match:
